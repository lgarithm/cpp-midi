    ...
    midiFile read(std::string filepath)
    {
        ...
        for (int track_id = 0; track_id < head.ntrks; ++track_id) {
            ...
            std::cout << "track " << track_id + 1
                      << ": length: " << tracks[track_id].length << std::endl;
#endif
  ...
  private:
    ...
    void get_events(std::ifstream &file, midi_track &track)
    {
        ...
        for (const unsigned char *p = byte; p < byte + track.length;) {
            ...
            case 0xF0:
            case 0xF7:
                e = get_sysex_event(status_byte, p);
                break;
            case 0xFF:
                e = get_meta_event(status_byte, p);
                break;
            default:
                e = get_midi_event(status_byte, p);
                break;
            }
    ...
    midi_event *get_midi_event(unsigned char status_byte,
                               const unsigned char *&p)
    {
        ...
        switch (type) {
        case 0x80:
            e = new note_off(p[0], p[1]);
            break;
        case 0x90:
            e = new note_on(p[0], p[1]);
            break;
        case 0xA0:
            e = new note_aftertouch(p[0], p[1]);
            break;
        case 0xB0:
            e = new controller(p[0], p[1]);
            break;
        case 0xC0:
            e = new program_change(p[0]);
            break;
        case 0xD0:
            e = new channel_aftertouch(p[0]);
            break;
        case 0xE0:
            e = new pitch_bend(p[0], p[1]);
            break;
    ...
    sysex_event *get_sysex_event(unsigned char status_byte,
                                 const unsigned char *&p)
    {
    ...
    meta_event *get_meta_event(unsigned char status_byte,
                               const unsigned char *&p)
    {
        // std::cout<<"(meta event)";
        ...
        switch (type) {
        case 0x00:
            e = new sequence_number(p[0], p[1]);
            break;
        case 0x01:
            e = new text_event(length, p);
            break;
        case 0x02:
            e = new copyright_notice(length, p);
            break;
        case 0x03:
            e = new sequence_or_track_name(length, p);
            break;
        case 0x04:
            e = new instrument_name(length, p);
            break;
        case 0x05:
            e = new lyrics(length, p);
            break;
        case 0x06:
            e = new marker(length, p);
            break;
        case 0x07:
            e = new cue_point(length, p);
            break;
        case 0x20:
            e = new midi_channel_prefix(p[0]);
            break;
        case 0x2F:
            e = new end_of_track();
            break;
        case 0x51:
            ...
            break;
        case 0x54:
            e = new smpte_offset(p);
            break;
        case 0x58:
            e = new time_signature(p[0], p[1], p[2], p[3]);
            break;
        case 0x59:
            e = new key_signature(p[0], p[1]);
            break;
        case 0x7F:
            e = new sequence_specific(length, p);
            break;

        default:
            e = new unknown_meta_event(type, length, p);
