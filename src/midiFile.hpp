...
class midi_track : public midi_chunk
{
  public:
    ...
    void add_event(event *e)
    {
        if (e != NULL) {
            if (set_end_of_track)
                warning.print("warning: end_of_track event has been set.");
            if (e->get_status_byte() == 0xFF) {
                meta_event *m = (meta_event *)e;
                if (m->get_status_byte() == 0x2F) { set_end_of_track = true; }
            }
            events.push_back(e);
        }
    }

    std::list<event *> get_events() const { return events; }
...
class midiFile
{
  public:
    ...
    midi_head get_head() const { return head; }

    std::vector<midi_track> get_tracks() const { return tracks; }
